...
interface ReportData {
  ...
  risk_score?: number;
  score_riesgo?: number;
  telefono_sospechoso?: string;
  banco_recaudo?: string;
  [key: string]: unknown;
}

export default function AIReasoningPanel({ selectedReport }: AIReasoningPanelProps) {
  ...
  const r: ReportData = selectedReport || defaultReport;
  const level = getRiskLevel((r.score_riesgo ?? r.risk_score ?? 0) as number);
