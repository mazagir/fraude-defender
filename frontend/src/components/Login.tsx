...
export function Login({ onLogin }: LoginProps) {
  ...
  const [mensaje, setMensaje] = useState<MessageState | null>(null);

  const loginConRuta = async (url: string, payload: Record<string, string | undefined>) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setMensaje({
        tipo: "error",
        texto: String((e as Error)?.message || e),
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      setMensaje({
        tipo: "error",
        texto: `Error registrando usuario: ${String((e as Error)?.message || e)}`,
      });
    } finally {
      setLoading(false);
    }
